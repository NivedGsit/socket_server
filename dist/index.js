...
const connectedUsers = {}; // socketId -> {role, userId}
...
io.on("connection", async (socket) => {
    ...
    if (role === "admin") {
        ...
        for (const userId of currentUsers) {
            ...
            const unreadRaw = await redisClient.get(`user:${userId}:unreadCount`);
    ...
    socket.on("get-all-chats", async () => {
        ...
        for (const key of keys) {
            ...
            // unread
            const unreadCount = parseInt((await redisClient.get(`user:${userId}:unreadCount`)) || "0");
            everyChats.push({
                userId,
                messages,
                details,
    ...
    socket.on("private-message", async ({ to, message, name, organization, location, email }) => {
        ...
        if (sender.role === "user") {
            ...
            if (targetSocketId)
                io.to(targetSocketId).emit("message", { from: "admin", message, name, organization, location, email });
        }
    });
    ...
    socket.on("kick-user", async (userId) => {
        ...
        if (!userId)
            return;
        const socketId = Object.entries(connectedUsers).find(([_, u]) => u.userId === userId)?.[0];
        if (!socketId)
            return;
